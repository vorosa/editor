//======================================================================
//
// File: AssociativeOtherEndR_AOTHPropertySource.java
//
...
public class AssociativeOtherEndR_AOTHPropertySource implements IPropertySource {
	private ClassAsAssociatedOtherSide_c m_inst;
	private ModelClass_c[] m_ClassRelated;
	private ReferredToIdentifierAttribute_c[] m_ReferencedIdentifier;
	...
	public AssociativeOtherEndR_AOTHPropertySource(
			ClassAsAssociatedOtherSide_c inst) {
		m_inst = inst;
	}
	/* (non-Javadoc)
	...
	public IPropertyDescriptor[] getPropertyDescriptors() {
		...
		ModelClass_c[] v_classRelated = ModelClass_c
				.getManyO_OBJsOnR201(ClassInAssociation_c.getManyR_OIRsOnR203(ReferredToClassInAssoc_c
						.getManyR_RTOsOnR204((ClassAsAssociatedOtherSide_c) m_inst)));

		m_ClassRelated = v_classRelated;
		num_children += m_ClassRelated.length;

		sorter.sort(v_classRelated);
		ReferredToIdentifierAttribute_c[] v_referencedidentifier = ReferredToIdentifierAttribute_c
				.getManyO_RTIDAsOnR110(ReferredToClassInAssoc_c
						.getManyR_RTOsOnR204((ClassAsAssociatedOtherSide_c) m_inst));

		m_ReferencedIdentifier = v_referencedidentifier;
		num_children += m_ReferencedIdentifier.length;

		sorter.sort(v_referencedidentifier);
		if (m_propertyDescriptors == null
			...
			for (int i = 0; i < m_ReferencedIdentifier.length; ++i) {
				m_propertyDescriptors[d_index] = new PropertyDescriptor(
						"ReferencedIdentifier." + String.valueOf(i),
						"Referred To Identifier Attribute");
				m_propertyDescriptors[d_index]
						.setCategory("Referenced Identifier");
				++d_index;
			}
	...
	public Object getPropertyValue(Object id) {
		...
		for (int i = 0; i < m_ReferencedIdentifier.length; ++i) {
			if (m_propertyDescriptors[d_index].getId().equals(id)) {
				return new ReferencedIdentifierO_RTIDAPropertySource(
						m_ReferencedIdentifier[i]);
			}
			++d_index;
		}
		...
		try {
			if (id.equals("Mult")) {
				...
				if (new_val != m_inst.getMult()) {
					trans_name = "Change in property: 'Mult' of Class As Associated Other Side";//$NON-NLS-1$
				...
				if (new_val != m_inst.getCond()) {
					trans_name = "Change in property: 'Cond' of Class As Associated Other Side";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
				...
				if (!value.toString().equals(m_inst.getTxt_phrs())) {
					trans_name = "Change in property: 'Txt_Phrs' of Class As Associated Other Side";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
