//======================================================================
//
// File: InstancesI_INSPropertySource.java
//
...
public class InstancesI_INSPropertySource implements IPropertySource {
	private Instance_c m_inst;
	private PropertyDescriptor[] m_propertyDescriptors;

	MetadataSortingManager sorter = MetadataSortingManager.createDefault();

	public InstancesI_INSPropertySource(Instance_c inst) {
		m_inst = inst;
	}
	/* (non-Javadoc)
	...
	public Object getPropertyValue(Object id) {
		...
		} else if (id.equals("Default_Name")) {
			return m_inst.getDefault_name();
		}
		return null;
	}
		...
		try {
			if (id.equals("Name")) {
				if (!value.toString().equals(m_inst.getName())) {
					trans_name = "Change in property: 'Name' of Instance";//$NON-NLS-1$
				...
				if (!value.toString().equals(m_inst.getDefault_name())) {
					trans_name = "Change in property: 'Default_Name' of Instance";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
					m_inst.setDefault_name(value.toString());
				}
			}
