//======================================================================
//
// File: FormalInstanceAttributeValuesSQ_AVPropertySource.java
//
...
public class FormalInstanceAttributeValuesSQ_AVPropertySource
		implements
			IPropertySource {
	private InstanceAttributeValue_c m_inst;
	private Attribute_c[] m_AttributeFormalAttribute;
	private PropertyDescriptor[] m_propertyDescriptors;

	MetadataSortingManager sorter = MetadataSortingManager.createDefault();

	public FormalInstanceAttributeValuesSQ_AVPropertySource(
			InstanceAttributeValue_c inst) {
		m_inst = inst;
	}
	/* (non-Javadoc)
	...
	public IPropertyDescriptor[] getPropertyDescriptors() {
		...
		int num_children = 0;
		Attribute_c[] v_attributeFormalAttribute = Attribute_c
				.getManyO_ATTRsOnR938((InstanceAttributeValue_c) m_inst);

		m_AttributeFormalAttribute = v_attributeFormalAttribute;
		num_children += m_AttributeFormalAttribute.length;

		sorter.sort(v_attributeFormalAttribute);
		if (m_propertyDescriptors == null
			...
			m_propertyDescriptors[1] = new TextPropertyDescriptor(
					"InformalName", "Informal Attribute Name");
			m_propertyDescriptors[1]
					.setCategory(BridgepointPropertySheetPage.basicCategoryName);
			m_propertyDescriptors[2] = new DescriptionPropertyDescriptor(
					"Descrip", "Instance Attribute Description", m_inst);
			...
			int d_index = 4;
			for (int i = 0; i < m_AttributeFormalAttribute.length; ++i) {
				m_propertyDescriptors[d_index] = new PropertyDescriptor(
						"AttributeFormalAttribute." + String.valueOf(i),
						"Formal Attribute");
		...
		try {
			if (id.equals("Value")) {
				if (!value.toString().equals(m_inst.getValue())) {
					trans_name = "Change in property: 'Value' of Instance Attribute Value";//$NON-NLS-1$
				...
				if (!value.toString().equals(m_inst.getInformalname())) {
					trans_name = "Change in property: 'InformalName' of Instance Attribute Value";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
				...
				if (!value.toString().equals(m_inst.getDescrip())) {
					trans_name = "Change in property: 'Descrip' of Instance Attribute Value";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
