//======================================================================
//
// File: OperationParameterO_TPARMPropertySource.java
//
...
public class OperationParameterO_TPARMPropertySource implements IPropertySource {
	private OperationParameter_c m_inst;
	...
	public OperationParameterO_TPARMPropertySource(OperationParameter_c inst) {
		m_inst = inst;
	}
	/* (non-Javadoc)
	...
	public IPropertyDescriptor[] getPropertyDescriptors() {
		...
		if (m_propertyDescriptors == null
			...
			DataType_c[] set10 = new DataType_c[0];
			DataType_c[] set10_2 = new DataType_c[0];
			// Note that datatypes our handled by the dt chooser dialog
			List list11 = new ArrayList(set10.length + set10_2.length);
			for (int i = 0; i < set10.length; ++i) {
				if (m_inst.Isallowedtype(set10[i].getName())) {
					list11.add(set10[i].getName());
				}
			}
			Collections.sort(list11, new Comparator() {
				Collator coll = Collator.getInstance(Locale.getDefault());
				public int compare(Object a, Object b) {
					return coll.compare((String) a, (String) b);
				}
			});
			m_DataTypeType = new String[list11.size()];
			for (int i = 0; i < list11.size(); ++i) {
				m_DataTypeType[i] = (String) list11.get(i);
			}
			m_propertyDescriptors[d_index] = new ChooserPropertyDescriptor(
					"DataType",
					"Type",
					m_inst,
					"com.mentor.nucleus.bp.core.ui.actions.SetTypeOnO_TPARMAction",
					readonly);
	...
	public Object getPropertyValue(Object id) {
		...
		if (id.equals("DataType")) {
			DataType_c val = DataType_c
					.getOneS_DTOnR118((OperationParameter_c) m_inst);
		...
		try {
			if (id.equals("Name")) {
				if (!value.toString().equals(m_inst.getName())) {
					trans_name = "Change in property: 'Name' of Operation Parameter";//$NON-NLS-1$
				...
				if (new_val != m_inst.getBy_ref()) {
					trans_name = "Change in property: 'By_Ref' of Operation Parameter";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
				...
				if (!value.toString().equals(m_inst.getDimensions())) {
					trans_name = "Change in property: 'Dimensions' of Operation Parameter";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
				...
				if (!value.toString().equals(m_inst.getDescrip())) {
					trans_name = "Change in property: 'Descrip' of Operation Parameter";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
			...
			if (id.equals("DataType")) {
				...
				if (!value.equals(val.getName())) {
					DataType_c[] set12_1 = new DataType_c[0];
					DataType_c[] set12_2 = new DataType_c[0];
					if (m_inst.isInGenericPackage()) {
						...
						Domain_c domain = Domain_c
								.getOneS_DOMOnR1(Subsystem_c.getOneS_SSOnR2(ModelClass_c.getOneO_OBJOnR115(Operation_c
										.getOneO_TFROnR117((OperationParameter_c) m_inst))));
						set12_1 = DataType_c.getManyS_DTsOnR14(domain);
						set12_2 = DataType_c
					...
					DataType_c[] set12_3 = new DataType_c[0];
					...
					DataType_c[] set12 = new DataType_c[set12_1.length
							+ set12_2.length + +set12_3.length];
					System.arraycopy(set12_1, 0, set12, 0, set12_1.length);
					System.arraycopy(set12_2, 0, set12, set12_1.length,
							set12_2.length);
					System.arraycopy(set12_3, 0, set12, set12_1.length
							+ set12_2.length, set12_3.length);
					for (int i = 0; i < set12.length; ++i) {
						if (value.equals(set12[i].getName())) {
							trans_name = "Change in property: 'Type' of Operation Parameter"; //$NON-NLS-1$
							tr = tm.startTransaction(trans_name,
									Ooaofooa.getDefaultInstance());
							m_inst.unrelateAcrossR118From(val);
							m_inst.relateAcrossR118To(set12[i]);
							value_changed = true;
							break;
						}
					}
