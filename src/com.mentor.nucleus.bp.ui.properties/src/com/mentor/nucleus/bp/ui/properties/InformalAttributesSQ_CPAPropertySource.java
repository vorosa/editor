//======================================================================
//
// File: InformalAttributesSQ_CPAPropertySource.java
//
...
public class InformalAttributesSQ_CPAPropertySource implements IPropertySource {
	private ClassParticipantAttribute_c m_inst;
	private PropertyDescriptor[] m_propertyDescriptors;

	MetadataSortingManager sorter = MetadataSortingManager.createDefault();

	public InformalAttributesSQ_CPAPropertySource(
			ClassParticipantAttribute_c inst) {
		m_inst = inst;
	}
	/* (non-Javadoc)
		...
		try {
			if (id.equals("Name")) {
				if (!value.toString().equals(m_inst.getName())) {
					trans_name = "Change in property: 'Name' of Class Participant Attribute";//$NON-NLS-1$
				...
				if (!value.toString().equals(m_inst.getType())) {
					trans_name = "Change in property: 'Type' of Class Participant Attribute";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
					m_inst.setType(value.toString());
				}
			} else if (id.equals("Descrip")) {
				if (!value.toString().equals(m_inst.getDescrip())) {
					trans_name = "Change in property: 'Descrip' of Class Participant Attribute";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
