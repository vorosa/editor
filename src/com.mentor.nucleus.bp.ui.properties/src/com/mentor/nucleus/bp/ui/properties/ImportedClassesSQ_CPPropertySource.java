//======================================================================
//
// File: ImportedClassesSQ_CPPropertySource.java
//
...
public class ImportedClassesSQ_CPPropertySource implements IPropertySource {
	private ClassParticipant_c m_inst;
	private Lifespan_c[] m_Lifespans;
	private ClassParticipantAttribute_c[] m_InformalAttributes;
	...
	public IPropertyDescriptor[] getPropertyDescriptors() {
		...
		Lifespan_c[] v_lifespans = Lifespan_c
				.getManySQ_LSsOnR940(InteractionParticipant_c
						.getManySQ_PsOnR930((ClassParticipant_c) m_inst));

		m_Lifespans = v_lifespans;
		num_children += m_Lifespans.length;

		sorter.sort(v_lifespans);
		ClassParticipantAttribute_c[] v_informalattributes = ClassParticipantAttribute_c
				.getManySQ_CPAsOnR947(InformalAttribute_c.getManySQ_IAsOnR947(ClassParticipantAttribute_c
						.getManySQ_CPAsOnR935((ClassParticipant_c) m_inst)));

		m_InformalAttributes = v_informalattributes;
		num_children += m_InformalAttributes.length;

		sorter.sort(v_informalattributes);
		...
		ModelClass_c[] v_classFormalClass = ModelClass_c
				.getManyO_OBJsOnR939((ClassParticipant_c) m_inst);

		m_ClassFormalClass = v_classFormalClass;
		num_children += m_ClassFormalClass.length;

		sorter.sort(v_classFormalClass);
		if (m_propertyDescriptors == null
			...
			m_propertyDescriptors[0] = new TextPropertyDescriptor(
					"InformalName", "Informal Class Name");
			...
			for (int i = 0; i < m_InformalAttributes.length; ++i) {
				m_propertyDescriptors[d_index] = new PropertyDescriptor(
						"InformalAttributes." + String.valueOf(i),
						"Class Participant Attribute");
				m_propertyDescriptors[d_index]
						.setCategory("Informal Attributes");
				++d_index;
			}
			...
			for (int i = 0; i < m_ClassFormalClass.length; ++i) {
				m_propertyDescriptors[d_index] = new PropertyDescriptor(
						"ClassFormalClass." + String.valueOf(i), "Formal Class");
	...
	public Object getPropertyValue(Object id) {
		...
		for (int i = 0; i < m_InformalAttributes.length; ++i) {
			if (m_propertyDescriptors[d_index].getId().equals(id)) {
				return new InformalAttributesSQ_CPAPropertySource(
						m_InformalAttributes[i]);
			}
			++d_index;
		}
		...
		for (int i = 0; i < m_ClassFormalClass.length; ++i) {
			if (m_propertyDescriptors[d_index].getId().equals(id)) {
				return new ClassO_OBJPropertySource(m_ClassFormalClass[i]);
			}
			++d_index;
		}
		...
		try {
			if (id.equals("InformalName")) {
				if (!value.toString().equals(m_inst.getInformalname())) {
					trans_name = "Change in property: 'InformalName' of Class Participant";//$NON-NLS-1$
				...
				if (!value.toString().equals(m_inst.getDescrip())) {
					trans_name = "Change in property: 'Descrip' of Class Participant";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
