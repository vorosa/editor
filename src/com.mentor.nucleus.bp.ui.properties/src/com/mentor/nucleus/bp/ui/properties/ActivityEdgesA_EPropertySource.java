//======================================================================
//
// File: ActivityEdgesA_EPropertySource.java
//
...
public class ActivityEdgesA_EPropertySource implements IPropertySource {
	private ActivityEdge_c m_inst;
	private PropertyDescriptor[] m_propertyDescriptors;

	MetadataSortingManager sorter = MetadataSortingManager.createDefault();

	public ActivityEdgesA_EPropertySource(ActivityEdge_c inst) {
		m_inst = inst;
	}
	/* (non-Javadoc)
	...
	public IPropertyDescriptor[] getPropertyDescriptors() {
		...
		if (m_propertyDescriptors == null || 2 != m_propertyDescriptors.length) {
			...
			m_propertyDescriptors[0] = new TextPropertyDescriptor("Guard",
					"Guard Condition");
			...
			m_propertyDescriptors[1] = new DescriptionPropertyDescriptor(
					"Descrip", "Edge Description", m_inst);
	...
	public void setPropertyValue(Object id, Object value) {
		...
		try {
			if (id.equals("Guard")) {
				if (!value.toString().equals(m_inst.getGuard())) {
					trans_name = "Change in property: 'Guard' of Activity Edge";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
					m_inst.setGuard(value.toString());
				}
			} else if (id.equals("Descrip")) {
				if (!value.toString().equals(m_inst.getDescrip())) {
					trans_name = "Change in property: 'Descrip' of Activity Edge";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
