//======================================================================
//
// File: FunctionS_SYNCPropertySource.java
//
...
public class FunctionS_SYNCPropertySource implements IPropertySource {
	private Function_c m_inst;
	private String[] m_DataTypeReturnType;
	private FunctionParameter_c[] m_FunctionParameter;
	...
	public FunctionS_SYNCPropertySource(Function_c inst) {
		m_inst = inst;
	}
	/* (non-Javadoc)
	...
	public IPropertyDescriptor[] getPropertyDescriptors() {
		...
		int num_children = 0;
		FunctionParameter_c[] v_functionparameter = FunctionParameter_c
				.getManyS_SPARMsOnR24((Function_c) m_inst);

		m_FunctionParameter = v_functionparameter;
		num_children += m_FunctionParameter.length;

		sorter.sort(v_functionparameter);
		if (m_propertyDescriptors == null
			...
			m_propertyDescriptors[3]
					.setCategory(BridgepointPropertySheetPage.basicCategoryName);
			m_propertyDescriptors[4] = new DimensionsPropertyDescriptor(
					"Return_Dimensions", "Return Array Dimensions", readonly
							|| m_inst.Isvoid());
			m_propertyDescriptors[4].setValidator(new DimensionsValidator(
					(NonRootModelElement) m_inst));
			m_propertyDescriptors[4].setDescription(DimensionsValidator
			...
			DataType_c[] set1 = new DataType_c[0];
			DataType_c[] set1_2 = new DataType_c[0];
			// Note that datatypes our handled by the dt chooser dialog
			List list2 = new ArrayList(set1.length + set1_2.length);
			for (int i = 0; i < set1.length; ++i) {
				if (m_inst.Isallowedreturntype(set1[i].getName())) {
					list2.add(set1[i].getName());
				}
			}
			...
			m_DataTypeReturnType = new String[list2.size()];
			for (int i = 0; i < list2.size(); ++i) {
				m_DataTypeReturnType[i] = (String) list2.get(i);
			}
			m_propertyDescriptors[d_index] = new ChooserPropertyDescriptor(
					"DataType",
					"Return Type",
					m_inst,
					"com.mentor.nucleus.bp.core.ui.actions.SetReturnTypeOnS_SYNCAction",
					readonly);
			m_propertyDescriptors[d_index]
					.setCategory(BridgepointPropertySheetPage.basicCategoryName);
			++d_index;
			for (int i = 0; i < m_FunctionParameter.length; ++i) {
				m_propertyDescriptors[d_index] = new PropertyDescriptor(
						"FunctionParameter." + String.valueOf(i),
						"Function Parameter");
				m_propertyDescriptors[d_index]
	...
	public Object getPropertyValue(Object id) {
		...
		if (id.equals("DataType")) {
			DataType_c val = DataType_c.getOneS_DTOnR25((Function_c) m_inst);
		...
		try {
			if (id.equals("Name")) {
				if (!value.toString().equals(m_inst.getName())) {
					trans_name = "Change in property: 'Name' of Function";//$NON-NLS-1$
				...
				if (!value.toString().equals(m_inst.getDescrip())) {
					trans_name = "Change in property: 'Descrip' of Function";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
				...
				if (new_val != m_inst.getSuc_pars()) {
					trans_name = "Change in property: 'Suc_Pars' of Function";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
				...
				if (!value.toString().equals(m_inst.getReturn_dimensions())) {
					trans_name = "Change in property: 'Return_Dimensions' of Function";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
			...
			if (id.equals("DataType")) {
				DataType_c val = DataType_c
						.getOneS_DTOnR25((Function_c) m_inst);

				if (!value.equals(val.getName())) {
					DataType_c[] set3_1 = new DataType_c[0];
					DataType_c[] set3_2 = new DataType_c[0];
					if (m_inst.isInGenericPackage()) {
						...
						Domain_c domain = Domain_c
								.getOneS_DOMOnR23((Function_c) m_inst);
						set3_1 = DataType_c.getManyS_DTsOnR14(domain);
						set3_2 = DataType_c
					...
					DataType_c[] set3_3 = new DataType_c[0];
					...
					DataType_c[] set3 = new DataType_c[set3_1.length
							+ set3_2.length + +set3_3.length];
					System.arraycopy(set3_1, 0, set3, 0, set3_1.length);
					System.arraycopy(set3_2, 0, set3, set3_1.length,
							set3_2.length);
					System.arraycopy(set3_3, 0, set3, set3_1.length
							+ set3_2.length, set3_3.length);
					for (int i = 0; i < set3.length; ++i) {
						if (value.equals(set3[i].getName())) {
							trans_name = "Change in property: 'Return Type' of Function"; //$NON-NLS-1$
							tr = tm.startTransaction(trans_name,
									Ooaofooa.getDefaultInstance());
							m_inst.unrelateAcrossR25From(val);
							m_inst.relateAcrossR25To(set3[i]);
							value_changed = true;
							break;
						}
					}
