//======================================================================
//
// File: ActivityPartitionsA_APPropertySource.java
//
...
public class ActivityPartitionsA_APPropertySource implements IPropertySource {
	private ActivityPartition_c m_inst;
	private PropertyDescriptor[] m_propertyDescriptors;

	MetadataSortingManager sorter = MetadataSortingManager.createDefault();

	public ActivityPartitionsA_APPropertySource(ActivityPartition_c inst) {
		m_inst = inst;
	}
	/* (non-Javadoc)
	...
	public IPropertyDescriptor[] getPropertyDescriptors() {
		...
		if (m_propertyDescriptors == null || 2 != m_propertyDescriptors.length) {
			...
			m_propertyDescriptors[1] = new DescriptionPropertyDescriptor(
					"Descrip", "Partition Description", m_inst);
		...
		try {
			if (id.equals("Name")) {
				if (!value.toString().equals(m_inst.getName())) {
					trans_name = "Change in property: 'Name' of Activity Partition";//$NON-NLS-1$
				...
				if (!value.toString().equals(m_inst.getDescrip())) {
					trans_name = "Change in property: 'Descrip' of Activity Partition";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
