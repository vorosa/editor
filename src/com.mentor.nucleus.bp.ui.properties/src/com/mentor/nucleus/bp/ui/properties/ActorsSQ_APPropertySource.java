//======================================================================
//
// File: ActorsSQ_APPropertySource.java
//
...
public class ActorsSQ_APPropertySource implements IPropertySource {
	private ActorParticipant_c m_inst;
	...
	public ActorsSQ_APPropertySource(ActorParticipant_c inst) {
		m_inst = inst;
	}
	/* (non-Javadoc)
	...
	public IPropertyDescriptor[] getPropertyDescriptors() {
		...
		Lifespan_c[] v_lifespans = Lifespan_c
				.getManySQ_LSsOnR940(InteractionParticipant_c
						.getManySQ_PsOnR930((ActorParticipant_c) m_inst));
		...
		if (m_propertyDescriptors == null
			...
			m_propertyDescriptors[1] = new DescriptionPropertyDescriptor(
					"Descrip", "Actor Description", m_inst);
		...
		try {
			if (id.equals("Name")) {
				if (!value.toString().equals(m_inst.getName())) {
					trans_name = "Change in property: 'Name' of Actor Participant";//$NON-NLS-1$
				...
				if (!value.toString().equals(m_inst.getDescrip())) {
					trans_name = "Change in property: 'Descrip' of Actor Participant";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
