//======================================================================
//
// File: LifespansSQ_LSPropertySource.java
//
...
public class LifespansSQ_LSPropertySource implements IPropertySource {
	private Lifespan_c m_inst;
	private TimingMark_c[] m_TimingMarks;
	...
	public LifespansSQ_LSPropertySource(Lifespan_c inst) {
		m_inst = inst;
	}
	/* (non-Javadoc)
	...
	public IPropertyDescriptor[] getPropertyDescriptors() {
		...
		int num_children = 0;
		TimingMark_c[] v_timingmarks = TimingMark_c
				.getManySQ_TMsOnR931((Lifespan_c) m_inst);

		m_TimingMarks = v_timingmarks;
		num_children += m_TimingMarks.length;

		sorter.sort(v_timingmarks);
		SynchronousMessage_c[] v_synchronousmessages = SynchronousMessage_c
				.getManyMSG_SMsOnR1018(Message_c
						.getManyMSG_MsOnR1007(InteractionParticipant_c
								.getManySQ_PsOnR930((Lifespan_c) m_inst)));
		...
		if (m_propertyDescriptors == null
			...
			m_propertyDescriptors[0] = new DescriptionPropertyDescriptor(
					"Descrip", "Communication Line Description", m_inst);
			...
			m_propertyDescriptors[1] = new BooleanPropertyDescriptor(
					"Destroyed", "Is Destructed", readonly);
			m_propertyDescriptors[1]
					.setCategory(BridgepointPropertySheetPage.basicCategoryName);

			int d_index = 2;
			for (int i = 0; i < m_TimingMarks.length; ++i) {
				m_propertyDescriptors[d_index] = new PropertyDescriptor(
						"TimingMarks." + String.valueOf(i), "Timing Mark");
				m_propertyDescriptors[d_index].setCategory("Timing Marks");
				++d_index;
			}
	...
	public Object getPropertyValue(Object id) {
		...
		} else if (id.equals("Destroyed")) {
			return new Boolean(m_inst.getDestroyed());
		}
		int d_index = 2;
		for (int i = 0; i < m_TimingMarks.length; ++i) {
			if (m_propertyDescriptors[d_index].getId().equals(id)) {
				return new TimingMarksSQ_TMPropertySource(m_TimingMarks[i]);
			}
			++d_index;
		}
		...
		try {
			if (id.equals("Descrip")) {
				if (!value.toString().equals(m_inst.getDescrip())) {
					trans_name = "Change in property: 'Descrip' of Lifespan";//$NON-NLS-1$
				...
				if (new_val != m_inst.getDestroyed()) {
					trans_name = "Change in property: 'Destroyed' of Lifespan"; //$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
					m_inst.setDestroyed(new_val);
				}
			}
