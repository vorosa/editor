//======================================================================
//
// File: StructureMemberS_MBRPropertySource.java
//
...
public class StructureMemberS_MBRPropertySource implements IPropertySource {
	private StructureMember_c m_inst;
	...
	public StructureMemberS_MBRPropertySource(StructureMember_c inst) {
		m_inst = inst;
	}
	/* (non-Javadoc)
	...
	public IPropertyDescriptor[] getPropertyDescriptors() {
		...
		if (m_propertyDescriptors == null
			...
			DataType_c[] set34 = new DataType_c[0];
			DataType_c[] set34_2 = new DataType_c[0];
			// Note that datatypes our handled by the dt chooser dialog
			List list35 = new ArrayList(set34.length + set34_2.length);
			for (int i = 0; i < set34.length; ++i) {
				if (m_inst.Isallowedtype(set34[i].getName())) {
					list35.add(set34[i].getName());
				}
			}
			Collections.sort(list35, new Comparator() {
				Collator coll = Collator.getInstance(Locale.getDefault());
				public int compare(Object a, Object b) {
					return coll.compare((String) a, (String) b);
				}
			});
			m_DataTypeType = new String[list35.size()];
			for (int i = 0; i < list35.size(); ++i) {
				m_DataTypeType[i] = (String) list35.get(i);
			}
			m_propertyDescriptors[d_index] = new ChooserPropertyDescriptor(
					"DataType",
					"Type",
					m_inst,
					"com.mentor.nucleus.bp.core.ui.actions.SetTypeOnS_MBRAction",
					readonly);
	...
	public Object getPropertyValue(Object id) {
		...
		if (id.equals("DataType")) {
			DataType_c val = DataType_c
					.getOneS_DTOnR45((StructureMember_c) m_inst);
		...
		try {
			if (id.equals("Name")) {
				if (!value.toString().equals(m_inst.getName())) {
					trans_name = "Change in property: 'Name' of Structure Member";//$NON-NLS-1$
				...
				if (!value.toString().equals(m_inst.getDescrip())) {
					trans_name = "Change in property: 'Descrip' of Structure Member";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
				...
				if (!value.toString().equals(m_inst.getDimensions())) {
					trans_name = "Change in property: 'Dimensions' of Structure Member";//$NON-NLS-1$
					tr = tm.startTransaction(trans_name,
							Ooaofooa.getDefaultInstance());
					value_changed = true;
			...
			if (id.equals("DataType")) {
				...
				if (!value.equals(val.getName())) {
					DataType_c[] set36_1 = new DataType_c[0];
					DataType_c[] set36_2 = new DataType_c[0];
					if (m_inst.isInGenericPackage()) {
						...
						Domain_c domain = Domain_c.getOneS_DOMOnR14(DataType_c
								.getOneS_DTOnR45((StructureMember_c) m_inst));
						set36_1 = DataType_c.getManyS_DTsOnR14(domain);
						set36_2 = DataType_c
					...
					DataType_c[] set36_3 = new DataType_c[0];
					...
					DataType_c[] set36 = new DataType_c[set36_1.length
							+ set36_2.length + +set36_3.length];
					System.arraycopy(set36_1, 0, set36, 0, set36_1.length);
					System.arraycopy(set36_2, 0, set36, set36_1.length,
							set36_2.length);
					System.arraycopy(set36_3, 0, set36, set36_1.length
							+ set36_2.length, set36_3.length);
					for (int i = 0; i < set36.length; ++i) {
						if (value.equals(set36[i].getName())) {
							trans_name = "Change in property: 'Type' of Structure Member"; //$NON-NLS-1$
							tr = tm.startTransaction(trans_name,
									Ooaofooa.getDefaultInstance());
							m_inst.unrelateAcrossR45From(val);
							m_inst.relateAcrossR45To(set36[i]);
							value_changed = true;
							break;
						}
					}
