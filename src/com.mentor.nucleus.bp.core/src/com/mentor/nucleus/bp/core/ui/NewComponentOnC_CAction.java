...
//======================================================================
//
// File: com/mentor/nucleus/bp/core/ui/NewComponentOnC_CAction.java
//
...
public class NewComponentOnC_CAction implements IObjectActionDelegate {

	/**
	 * Constructor for NewComponentOnC_CAction.
	 */
	public NewComponentOnC_CAction() {
		super();
	}
	/**
	...
	public void run(IAction action) {
		...
		C_C_NewComponent(structuredSelection);
	...
	private void C_C_NewComponent(IStructuredSelection selection) {
		...
		try {
			...
			if ((!(v_component == null))) {
				...
				java.util.UUID v_discardUuid = v_component.Newcomponent();

			}

			if (v_component.isInGenericPackage()) {
				NonRootModelElement[] children = Component_c
						.getManyC_CsOnR8001(PackageableElement_c
								.getManyPE_PEsOnR8003(v_component))
				...
				if (!option) {
					...
					if (performRename) {
						...
						RenameActionUtil.setElementName(newElement,
								proposedName);
			...
			CorePlugin.logError("Transaction: New Component failed", e);//$NON-NLS-1$
		}
		if ((!CorePlugin
				.getDefault()
				.getPreferenceStore()
