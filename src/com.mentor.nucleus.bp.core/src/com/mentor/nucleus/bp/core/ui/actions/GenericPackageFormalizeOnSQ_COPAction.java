...
//======================================================================
//
// File: com/mentor/nucleus/bp/core/ui/GenericPackageFormalizeOnSQ_COPAction.java
//
...
public class GenericPackageFormalizeOnSQ_COPAction
		implements
			IObjectActionDelegate {

	/**
	 * Constructor for GenericPackageFormalizeOnSQ_COPAction.
	 */
	public GenericPackageFormalizeOnSQ_COPAction() {
		super();
	}
	/**
	...
	public void run(IAction action) {
		...
		SQ_COP_GenericPackageFormalize(structuredSelection);
	}
	/**
	...
	public static void SQ_COP_GenericPackageFormalize(
		...
		ComponentParticipant_c v_element = (ComponentParticipant_c) context;
		// select related by where USER::selectExisting
		Component_c v_existingElement = Component_c.getOneC_COnR955(v_element);
		...
		ElementSelectionDialog dialog = new ElementSelectionDialog(PlatformUI
				...
				elements, "component", true, v_existingElement, true, pkg);
		dialog.setBlockOnOpen(true);
		dialog.setTitle("Formalize Selection");
		int result = dialog.open();
		if (result == Window.OK) {
			Component_c v_selectedElement = (Component_c) dialog.getResult()[0];
				...
				try {
					...
					if (((v_element != null))) {
						...
						if (((v_selectedElement != null))) {
							...
							if (v_element != null) {
								v_element.Formalize(v_selectedElement.getId());
							} else {
	...
	public static Component_c[] getElements(ComponentParticipant_c v_element) {
        ...
        for(int i = 0; i < instancesUnderSystem.length; i++) {
    	  ...
    	  for(int j = 0; j < elementInstances.length; j++) {
		  	Component_c elem = elementInstances[j];
						if(instancesUnderSystem[i].getInstanceList(ComponentPackage_c.class).isEmpty()) {
			...
			for (int i = 0; i < systems.length; i++) {
				if (systems[i] != sys && systems[i].getUseglobals()) {
					...
					for (int j = 0; j < instancesUnderSystem.length; j++) {
						Component_c[] elementInstances = Component_c.ComponentInstances(instancesUnderSystem[j]);
						for (int k = 0; k < elementInstances.length; k++) {
							Component_c elem = elementInstances[k];
  ...
  } // end getElements(ComponentParticipant_c) 
} // end GenericPackageFormalizeOnSQ_COPAction
