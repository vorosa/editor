//========================================================================
//
// File: src/com/mentor/nucleus/bp/core/ui/cells/StructureMemberCellProvider.java
//
...
public class StructureMemberCellProvider implements ICellProvider {
	...
	public void setValue(NonRootModelElement element, ObjectElement data, Object value) {
        if(data.getName().equals("Name"))
        {
            if ( ! value.toString().equals(((StructureMember_c) element).getName()) )
            {
                ((StructureMember_c) element).setName(value.toString());
            }
        }
        if(data.getName().equals("Descrip"))
        {
            if ( ! value.toString().equals(((StructureMember_c) element).getDescrip()) )
            {
                ((StructureMember_c) element).setDescrip(value.toString());
            }
        }
        if(data.getName().equals("Dimensions"))
        {
            if ( ! value.toString().equals(((StructureMember_c) element).getDimensions()) )
            {
                Vector dims = DimensionsUtil.getDimensionsData(value.toString(),(StructureMember_c) element);
                if (dims.size() > 0) {
                    for (int i = 0; i < dims.size(); i++) {
                        int numElements = ((Integer)dims.get(i)).intValue();
                        ((StructureMember_c) element).Resizedimensions(i, numElements, dims.size());
                    }
                } else {
                    ((StructureMember_c) element).Resizedimensions(0, 0, 0);
                }
                ((StructureMember_c) element).setDimensions(value.toString());
            }
        }
