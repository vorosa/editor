...
//======================================================================
//
// File: com/mentor/nucleus/bp/core/ui/FormalizeOnSQ_PPAction.java
//
...
public class FormalizeOnSQ_PPAction implements IObjectActionDelegate {

	/**
	 * Constructor for FormalizeOnSQ_PPAction.
	 */
	public FormalizeOnSQ_PPAction() {
		super();
	}
	/**
	...
	public void run(IAction action) {
		...
		SQ_PP_Formalize(structuredSelection);
	}
	/**
	...
	public static void SQ_PP_Formalize(IStructuredSelection selection) {
		...
		PackageParticipant_c v_element = (PackageParticipant_c) context;
		// select related by where USER::selectExisting
		Package_c v_existingElement = Package_c.getOneEP_PKGOnR956(v_element);

		// ensure that all Packages are loaded
		PersistenceManager.ensureAllInstancesLoaded(v_element.getModelRoot(),
				Package_c.class);
		// now find all the elements that should be shown 
		Package_c[] elements = getElements(v_element);
		...
		ElementSelectionDialog dialog = new ElementSelectionDialog(PlatformUI
				...
				elements, "package", true, v_existingElement, true, pkg);
		dialog.setBlockOnOpen(true);
		dialog.setTitle("Formalize Selection");
		int result = dialog.open();
		if (result == Window.OK) {
			Package_c v_selectedElement = (Package_c) dialog.getResult()[0];
			if (v_selectedElement != null) {

				TransactionUtil.TransactionGroup transactionGroup = TransactionUtil
						.startTransactionsOnSelectedModelRoots("Package Formalize");
				try {
					...
					CorePlugin.logError(
							"Transaction: Package Formalize failed", e);//$NON-NLS-1$
				}
	...
	public static Package_c[] getElements(PackageParticipant_c v_element) {
      List<Package_c> elementList = new ArrayList<Package_c>();
      ...
      Component_c comp = Component_c.getOneC_COnR8003(pe);
        ...
        for(int i = 0; i < instancesUnderSystem.length; i++) {
    	  Package_c[] elementInstances = Package_c.PackageInstances(instancesUnderSystem[i]);
    	  for(int j = 0; j < elementInstances.length; j++) {
		  	Package_c elem = elementInstances[j];
		...
		if (sys.getUseglobals()) {
          ...
          Package_c[] elementInstances = Package_c.getManyEP_PKGsOnR8001(pes);
  	      for(int j = 0; j < elementInstances.length; j++) {
		  	Package_c elem = elementInstances[j];
			...
			for (int i = 0; i < systems.length; i++) {
				if (systems[i] != sys && systems[i].getUseglobals()) {
					...
					for (int j = 0; j < instancesUnderSystem.length; j++) {
						Package_c[] elementInstances = Package_c.PackageInstances(instancesUnderSystem[j]);
						for (int k = 0; k < elementInstances.length; k++) {
							Package_c elem = elementInstances[k];
	...
	return elementList.toArray(new Package_c[elementList.size()]);
  } // end getElements(PackageParticipant_c) 
} // end FormalizeOnSQ_PPAction
