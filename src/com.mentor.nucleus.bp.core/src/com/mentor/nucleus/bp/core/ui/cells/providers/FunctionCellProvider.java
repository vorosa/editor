//========================================================================
//
// File: src/com/mentor/nucleus/bp/core/ui/cells/FunctionCellProvider.java
//
...
public class FunctionCellProvider implements ICellProvider {
	...
	public void setValue(NonRootModelElement element, ObjectElement data, Object value) {
        if(data.getName().equals("Name"))
        {
            if ( ! value.toString().equals(((Function_c) element).getName()) )
            {
                ((Function_c) element).setName(value.toString());
            }
        }
        if(data.getName().equals("Descrip"))
        {
            if ( ! value.toString().equals(((Function_c) element).getDescrip()) )
            {
                ((Function_c) element).setDescrip(value.toString());
            }
        }
        if(data.getName().equals("Suc_Pars"))
        {
            int new_val = Integer.parseInt(value.toString());
            if ( new_val != ((Function_c) element).getSuc_pars() )
            {
                ((Function_c) element).setSuc_pars(new_val);
            }
        }
        if(data.getName().equals("Return_Dimensions"))
        {
            if ( ! value.toString().equals(((Function_c) element).getReturn_dimensions()) )
            {
                Vector dims = DimensionsUtil.getDimensionsData(value.toString(),(Function_c) element);
                if (dims.size() > 0) {
                    for (int i = 0; i < dims.size(); i++) {
                        int numElements = ((Integer)dims.get(i)).intValue();
                        ((Function_c) element).Resizereturn_dimensions(i, numElements, dims.size());
                    }
                } else {
                    ((Function_c) element).Resizereturn_dimensions(0, 0, 0);
                }
                ((Function_c) element).setReturn_dimensions(value.toString());
            }
        }
