...
//======================================================================
//
// File: com/mentor/nucleus/bp/core/ui/GenericPackageFormalizeOnSQ_EEPAction.java
//
...
public class GenericPackageFormalizeOnSQ_EEPAction
		implements
			IObjectActionDelegate {

	/**
	 * Constructor for GenericPackageFormalizeOnSQ_EEPAction.
	 */
	public GenericPackageFormalizeOnSQ_EEPAction() {
		super();
	}
	/**
	...
	public void run(IAction action) {
		...
		SQ_EEP_GenericPackageFormalize(structuredSelection);
	}
	/**
	...
	public static void SQ_EEP_GenericPackageFormalize(
		...
		ExternalEntityParticipant_c v_element = (ExternalEntityParticipant_c) context;
		// select related by where USER::selectExisting
		ExternalEntity_c v_existingElement = ExternalEntity_c
				.getOneS_EEOnR933(v_element);

		// ensure that all External Entitys are loaded
		PersistenceManager.ensureAllInstancesLoaded(v_element.getModelRoot(),
				ExternalEntity_c.class);
		// now find all the elements that should be shown 
		ExternalEntity_c[] elements = getElements(v_element);
		...
		ElementSelectionDialog dialog = new ElementSelectionDialog(PlatformUI
				...
				elements, "external entity", true, v_existingElement, true, pkg);
		dialog.setBlockOnOpen(true);
		dialog.setTitle("Formalize Selection");
		int result = dialog.open();
		if (result == Window.OK) {
			ExternalEntity_c v_selectedElement = (ExternalEntity_c) dialog
					.getResult()[0];
	...
	public static ExternalEntity_c[] getElements(ExternalEntityParticipant_c v_element) {
      List<ExternalEntity_c> elementList = new ArrayList<ExternalEntity_c>();
      ...
      Component_c comp = Component_c.getOneC_COnR8003(pe);
        ...
        for(int i = 0; i < instancesUnderSystem.length; i++) {
    	  ExternalEntity_c[] elementInstances = ExternalEntity_c.ExternalEntityInstances(instancesUnderSystem[i]);
    	  for(int j = 0; j < elementInstances.length; j++) {
		  	ExternalEntity_c elem = elementInstances[j];
		...
		if (sys.getUseglobals()) {
          ...
          ExternalEntity_c[] elementInstances = ExternalEntity_c.getManyS_EEsOnR8001(pes);
  	      for(int j = 0; j < elementInstances.length; j++) {
		  	ExternalEntity_c elem = elementInstances[j];
			...
			for (int i = 0; i < systems.length; i++) {
				if (systems[i] != sys && systems[i].getUseglobals()) {
					...
					for (int j = 0; j < instancesUnderSystem.length; j++) {
						ExternalEntity_c[] elementInstances = ExternalEntity_c.ExternalEntityInstances(instancesUnderSystem[j]);
						for (int k = 0; k < elementInstances.length; k++) {
							ExternalEntity_c elem = elementInstances[k];
	...
	return elementList.toArray(new ExternalEntity_c[elementList.size()]);
  } // end getElements(ExternalEntityParticipant_c) 
} // end GenericPackageFormalizeOnSQ_EEPAction
