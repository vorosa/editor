//========================================================================
//
// File: src/com/mentor/nucleus/bp/core/ui/cells/FunctionParameterCellProvider.java
//
...
public class FunctionParameterCellProvider implements ICellProvider {
	...
	public void setValue(NonRootModelElement element, ObjectElement data, Object value) {
        if(data.getName().equals("Name"))
        {
            if ( ! value.toString().equals(((FunctionParameter_c) element).getName()) )
            {
                ((FunctionParameter_c) element).setName(value.toString());
            }
        }
        if(data.getName().equals("By_Ref"))
        {
            int new_val = Integer.parseInt(value.toString());
            if ( new_val != ((FunctionParameter_c) element).getBy_ref() )
            {
                ((FunctionParameter_c) element).setBy_ref(new_val);
            }
        }
        if(data.getName().equals("Dimensions"))
        {
            if ( ! value.toString().equals(((FunctionParameter_c) element).getDimensions()) )
            {
                Vector dims = DimensionsUtil.getDimensionsData(value.toString(),(FunctionParameter_c) element);
                if (dims.size() > 0) {
                    for (int i = 0; i < dims.size(); i++) {
                        int numElements = ((Integer)dims.get(i)).intValue();
                        ((FunctionParameter_c) element).Resizedimensions(i, numElements, dims.size());
                    }
                } else {
                    ((FunctionParameter_c) element).Resizedimensions(0, 0, 0);
                }
                ((FunctionParameter_c) element).setDimensions(value.toString());
            }
        }
        if(data.getName().equals("Descrip"))
        {
            if ( ! value.toString().equals(((FunctionParameter_c) element).getDescrip()) )
            {
                ((FunctionParameter_c) element).setDescrip(value.toString());
            }
        }
