...
public class TypesStructuredDatatypeOnEP_PKGAction
		implements
			IObjectActionDelegate {

	/**
	 * Constructor for TypesStructuredDatatypeOnEP_PKGAction.
	 */
	public TypesStructuredDatatypeOnEP_PKGAction() {
		super();
	}
	/**
	...
	public void run(IAction action) {
		...
		EP_PKG_TypesStructuredDatatype(structuredSelection);
	...
	private void EP_PKG_TypesStructuredDatatype(IStructuredSelection selection) {
		...
		TransactionUtil.TransactionGroup transactionGroup = TransactionUtil
				.startTransactionsOnSelectedModelRoots("Types Structured DataType");
		boolean abortTransaction = false;
		try {
			// Ensure that actions take place between Verifier Activity executions
			Ooaofooa.beginSaveOperation();
			if (v_pkg != null) {
				v_pkg.Newstructureddatatype();
			} else {
			...
			NonRootModelElement[] children = StructuredDataType_c
			...
			CorePlugin.logError(
					"Transaction: Types Structured DataType failed", e);//$NON-NLS-1$
		}
		if ((!CorePlugin
				.getDefault()
				.getPreferenceStore()
...
} // end TypesStructuredDatatypeOnEP_PKGAction
